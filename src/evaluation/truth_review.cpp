...
[[nodiscard]] TruthReviewError review_error(const TruthReviewErrorCode code,
                                            std::string message) {
...
[[nodiscard]] constexpr std::string_view status_name(
    const core::RecordedValueStatus value) noexcept {
    switch (value) {
    case core::RecordedValueStatus::available: return "available";
    case core::RecordedValueStatus::unsupported: return "unsupported";
    case core::RecordedValueStatus::inaccessible: return "inaccessible";
...
[[nodiscard]] constexpr std::string_view source_name(
    const core::SystemEventSource value) noexcept {
    switch (value) {
    ...
    case core::SystemEventSource::service_control_manager: return "service";
    case core::SystemEventSource::defender: return "defender";
    case core::SystemEventSource::windows_update: return "windows_update";
...
[[nodiscard]] constexpr std::string_view kind_name(
    const core::SystemEventKind value) noexcept {
    constexpr std::array names{
        "suspend", "resume_automatic", "resume_user", "device_enumerated",
        "device_started", "device_removed", "audio_endpoint_added",
        "audio_endpoint_removed", "audio_endpoint_state_changed",
...
[[nodiscard]] constexpr std::string_view level_name(
    const core::SystemEventLevel value) noexcept {
    switch (value) {
    case core::SystemEventLevel::informational: return "informational";
...
template <typename Value>
void write_json_value(std::ostream& output,
...
[[nodiscard]] std::vector<ProcessReview> process_reviews(
    const core::IncidentSnapshot& incident, const bool include_identities) {
...
std::expected<TruthReviewStatistics, TruthReviewError> export_truth_review(
    const core::IncidentSnapshot& incident, const std::string_view incident_key,
    const std::int64_t created_utc_milliseconds,
    const std::filesystem::path& destination,
    const TruthReviewOptions options) noexcept {
    try {
        if (!valid_key(incident_key) || destination.empty() ||
        ...
        if (!parent.empty()) {
            ...
            if (issue) {
                return std::unexpected{review_error(TruthReviewErrorCode::io,
                                                    "cannot create truth-review parent")};
            }
        }
        if (!std::filesystem::create_directory(staging, issue) || issue) {
            return std::unexpected{review_error(TruthReviewErrorCode::io,
                                                "cannot create truth-review staging")};
        }
        ...
        const auto event_time = incident.header().window.event_time;
        {
            ...
            if (!stream_ok(output)) {
                return std::unexpected{review_error(TruthReviewErrorCode::io,
                                                    "cannot write truth-review manifest")};
            }
        }
        {
            std::ofstream output{staging / "system-samples.tsv", std::ios::binary};
            output << std::setprecision(17)
                   << "offset_seconds\tcpu_status\tcpu_fraction\tmemory_status\tmemory_fraction\t"
                      ...
                      "disk_service_status\tdisk_service_seconds\tdisk_queue_status\tdisk_queue\t"
                      "network_receive_status\tnetwork_receive_bps\tnetwork_transmit_status\t"
                      "network_transmit_bps\tretransmit_status\tretransmit_fraction\t"
                      "failed_connections_status\tfailed_connections\treset_status\tresets\t"
                      "gpu_status\tgpu_fraction\tdpc_status\tdpc_fraction\tinterrupt_status\t"
                      "interrupt_fraction\tfrequency_status\tfrequency_mhz\tthermal_status\t"
                      "thermal_fraction\tbattery_status\tbattery_fraction\tuptime_status\tuptime_seconds\n";
            for (const auto& sample : systems) {
                output << relative_seconds(sample.observed_at, event_time) << '\t';
                write_recorded(output, sample.cpu_fraction); output << '\t';
                write_recorded(output, sample.memory_fraction); output << '\t';
                write_recorded(output, sample.disk_read_bytes_per_second); output << '\t';
                write_recorded(output, sample.disk_write_bytes_per_second); output << '\t';
                write_recorded(output, sample.disk_read_latency_seconds); output << '\t';
                write_recorded(output, sample.disk_write_latency_seconds); output << '\t';
                write_recorded(output, sample.disk_service_time_seconds); output << '\t';
                write_recorded(output, sample.disk_queue_depth); output << '\t';
                write_recorded(output, sample.network_receive_bytes_per_second); output << '\t';
                write_recorded(output, sample.network_transmit_bytes_per_second); output << '\t';
                write_recorded(output, sample.network_tcp_retransmit_fraction); output << '\t';
                write_recorded(output, sample.network_tcp_failed_connections); output << '\t';
                write_recorded(output, sample.network_tcp_resets); output << '\t';
                write_recorded(output, sample.gpu_fraction); output << '\t';
                write_recorded(output, sample.dpc_fraction); output << '\t';
                write_recorded(output, sample.interrupt_fraction); output << '\t';
                write_recorded(output, sample.cpu_current_mhz); output << '\t';
                write_recorded(output, sample.cpu_thermal_limit_fraction); output << '\t';
                write_recorded(output, sample.battery_fraction); output << '\t';
                write_recorded(output, sample.system_uptime_seconds); output << '\n';
            }
            if (!stream_ok(output)) {
                return std::unexpected{review_error(TruthReviewErrorCode::io,
                                                    "cannot write system truth evidence")};
            }
        }
        {
            std::ofstream output{staging / "processes.tsv", std::ios::binary};
            output << std::setprecision(17)
                   << "process_ordinal\tlocal_pid\tlocal_name\tsample_count\tpeak_cpu_percent\t"
            ...
            for (const auto& process : processes) {
                ...
                output << '\t' << process.samples.size() << '\t'
                       << process.peak_cpu_percent << '\t'
                       << process.peak_working_set_mib << '\t'
                       << process.peak_disk_mib_per_second << '\n';
            }
            if (!stream_ok(output)) {
                return std::unexpected{review_error(TruthReviewErrorCode::io,
                                                    "cannot write process truth legend")};
            }
        }
        {
            ...
            for (const auto& process : processes) {
                for (const auto* sample : process.samples) {
                    output << relative_seconds(sample->observed_at, event_time) << '\t'
                           << process.ordinal << '\t';
                    write_recorded(output, sample->cpu_fraction); output << '\t';
                    write_recorded(output, sample->working_set_bytes); output << '\t';
                    write_recorded(output, sample->disk_read_bytes_per_second); output << '\t';
                    write_recorded(output, sample->disk_write_bytes_per_second); output << '\n';
                }
            }
            if (!stream_ok(output)) {
                return std::unexpected{review_error(TruthReviewErrorCode::io,
                                                    "cannot write process truth evidence")};
            }
        }
        {
            std::ofstream output{staging / "system-events.tsv", std::ios::binary};
            output << std::setprecision(17)
                   << "offset_seconds\tsource\tkind\tlevel\tnative_event_id\tdetail\tprocess_ordinal\n";
            for (const auto& event : events) {
                ...
                if (event.has_process_identity) {
                    const auto found = process_ordinals.find(
                        {event.process_pid, event.process_creation_token});
            ...
            if (!stream_ok(output)) {
                return std::unexpected{review_error(TruthReviewErrorCode::io,
                                                    "cannot write event truth evidence")};
            }
        }
        {
        ...
        }
        {
            ...
            output << std::setprecision(17)
                   << "<!doctype html><html lang=\"en\"><head><meta charset=\"utf-8\">"
                      ...
                      "body{font:15px system-ui;background:#10141c;color:#edf2f7;margin:24px;max-width:1400px}"
                      "h1,h2{color:#fff} .notice{padding:12px;border:1px solid #60708b;background:#182131}"
                      "canvas{width:100%;height:320px;background:#090d13;border:1px solid #334155}"
                      "select{background:#111827;color:#fff;padding:6px}table{border-collapse:collapse;width:100%}"
                      "th,td{border-bottom:1px solid #334155;padding:6px;text-align:left}small{color:#aebbd0}"
                      "</style></head><body><h1>Independent incident truth review</h1><div class=\"notice\">"
                      "This artifact contains recorded evidence only. It does not run or display BlackBox "
                      "predictions. Fix an independent ballot before viewing development prediction output."
                      "</div><p>Incident <code>" << incident_key << "</code>; event marker is at 0 seconds. "
                   << (options.include_local_process_identities
                           ? "This local artifact includes process names and PIDs; do not copy it into the corpus."
                           : "Process identity is represented only by incident-local ordinal.")
                   << "</p><h2>System timeline</h2><label>Metric <select id=\"metric\">"
                      "<option value=\"cpu\">CPU (%)</option><option value=\"memory\">Memory (%)</option>"
                      "<option value=\"diskLatency\">Disk read latency (ms)</option>"
                      "<option value=\"diskQueue\">Disk queue depth</option>"
                      "<option value=\"retransmit\">TCP retransmit (%)</option>"
                      "<option value=\"gpu\">GPU (%)</option><option value=\"dpc\">DPC (%)</option>"
                      "<option value=\"interrupt\">Interrupt (%)</option></select></label>"
                      "<canvas id=\"system\" width=\"1200\" height=\"320\"></canvas>"
                      "<h2>Process timeline</h2><label>Process <select id=\"process\">";
            for (const auto& process : processes) {
                output << "<option value=\"" << process.ordinal << "\">"
                       << html_text(process.label) << "</option>";
            }
            output << "</select></label><canvas id=\"processChart\" width=\"1200\" height=\"320\"></canvas>"
                      "<h2>Process evidence summary</h2><table><thead><tr><th>Ordinal</th><th>Local label</th>"
                      "<th>Samples</th><th>Peak CPU</th><th>Peak working set MiB</th><th>Peak disk MiB/s</th>"
                      "</tr></thead><tbody>";
            for (const auto& process : processes) {
                output << "<tr><td>" << process.ordinal << "</td><td>"
                       << html_text(process.label) << "</td><td>" << process.samples.size()
                       << "</td><td>" << process.peak_cpu_percent << "</td><td>"
                       << process.peak_working_set_mib << "</td><td>"
                       << process.peak_disk_mib_per_second << "</td></tr>";
            }
            output << "</tbody></table><h2>Recorded system events</h2><table><thead><tr><th>Time</th>"
                      "<th>Source</th><th>Kind</th><th>Level</th><th>Process</th></tr></thead><tbody>";
            for (const auto& event : events) {
            ...
            output << "</tbody></table><p><small>Unavailable values create gaps. Raw normalized rows are "
                      "retained beside this page. Complete one copy of ballot-template.tsv per independent "
                      "annotator; the session operator cannot be an annotator.</small></p><script>const sys=[";
            for (std::size_t index = 0U; index < systems.size(); ++index) {
                ...
                write_json_value(output, sample.cpu_fraction, 100.0); output << ",memory:";
                write_json_value(output, sample.memory_fraction, 100.0); output << ",diskLatency:";
                write_json_value(output, sample.disk_read_latency_seconds, 1'000.0); output << ",diskQueue:";
                write_json_value(output, sample.disk_queue_depth); output << ",retransmit:";
                write_json_value(output, sample.network_tcp_retransmit_fraction, 100.0); output << ",gpu:";
                write_json_value(output, sample.gpu_fraction, 100.0); output << ",dpc:";
                write_json_value(output, sample.dpc_fraction, 100.0); output << ",interrupt:";
                write_json_value(output, sample.interrupt_fraction, 100.0); output << '}';
            }
            output << "];const proc={";
            for (std::size_t process_index = 0U; process_index < processes.size(); ++process_index) {
                ...
                for (std::size_t index = 0U; index < process.samples.size(); ++index) {
                    ...
                    output << "{t:" << relative_seconds(sample.observed_at, event_time) << ",cpu:";
                    write_json_value(output, sample.cpu_fraction, 100.0); output << '}';
                }
                output << ']';
            }
            output << "};function draw(id,rows,key,label){const c=document.getElementById(id),x=c.getContext('2d');"
                      "x.clearRect(0,0,c.width,c.height);x.fillStyle='#dbeafe';x.fillText(label,12,18);"
                      "if(!rows.length)return;const ts=rows.map(r=>r.t),vals=rows.map(r=>r[key]).filter(v=>v!==null);"
                      "if(!vals.length){x.fillText('No available values',12,42);return;}const lo=Math.min(...ts),hi=Math.max(...ts);"
                      "const top=Math.max(...vals,0.000001),px=t=>40+(t-lo)/Math.max(hi-lo,.001)*(c.width-60),"
                      "py=v=>c.height-30-v/top*(c.height-60);x.strokeStyle='#38bdf8';x.lineWidth=2;x.beginPath();let open=false;"
                      "for(const r of rows){const v=r[key];if(v===null){open=false;continue;}if(!open){x.moveTo(px(r.t),py(v));open=true;}"
                      "else x.lineTo(px(r.t),py(v));}x.stroke();if(lo<=0&&hi>=0){x.strokeStyle='#fb7185';x.beginPath();"
                      "x.moveTo(px(0),25);x.lineTo(px(0),c.height-25);x.stroke();}x.fillStyle='#94a3b8';"
                      "x.fillText(lo.toFixed(1)+' s',35,c.height-8);x.fillText(hi.toFixed(1)+' s',c.width-70,c.height-8);"
                      "x.fillText('max '+top.toFixed(2),c.width-110,18);}const labels={cpu:'CPU (%)',memory:'Memory (%)',"
                      "diskLatency:'Disk read latency (ms)',diskQueue:'Disk queue depth',retransmit:'TCP retransmit (%)',"
                      "gpu:'GPU (%)',dpc:'DPC (%)',interrupt:'Interrupt (%)'};function drawSystem(){const k=document.getElementById('metric').value;"
                      "draw('system',sys,k,labels[k]);}function drawProcess(){const k=document.getElementById('process').value;"
                      "draw('processChart',proc[k]||[],'cpu','Process CPU (%)');}document.getElementById('metric').onchange=drawSystem;"
                      "document.getElementById('process').onchange=drawProcess;drawSystem();drawProcess();</script></body></html>";
            if (!stream_ok(output)) {
                return std::unexpected{review_error(TruthReviewErrorCode::io,
        ...
        for (std::filesystem::directory_iterator iterator{staging, issue}, end;
            ...
            if (issue || !std::filesystem::is_regular_file(status) ||
                ...
                return std::unexpected{review_error(TruthReviewErrorCode::io,
                                                    "truth-review staging is not exact")};
            }
        ...
        if (issue) {
            return std::unexpected{review_error(TruthReviewErrorCode::io,
                                                "cannot publish truth-review directory")};
        }
        ...
        return std::unexpected{review_error(TruthReviewErrorCode::io,
                                            "unknown truth-review export failure")};
    }
}
