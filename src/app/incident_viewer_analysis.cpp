...
resource_name(const analysis::ResourceKind resource) noexcept {
    switch (resource) {
...
[[nodiscard]] constexpr std::string_view metric_name(const analysis::MetricKind metric) noexcept {
    switch (metric) {
    ...
    case analysis::MetricKind::disk_read_latency:
        return "physical-disk read latency";
    case analysis::MetricKind::disk_write_latency:
        return "physical-disk write latency";
    case analysis::MetricKind::disk_service_time:
        return "physical-disk service time";
...
[[nodiscard]] constexpr std::string_view
confidence_name(const analysis::AnalysisConfidence confidence) noexcept {
    switch (confidence) {
    case analysis::AnalysisConfidence::unavailable:
        return "Unavailable";
    case analysis::AnalysisConfidence::low:
        return "Low / cold start";
    case analysis::AnalysisConfidence::moderate:
        return "Moderate";
    case analysis::AnalysisConfidence::high:
        return "High";
    }
    return "Unavailable";
}
...
[[nodiscard]] constexpr std::string_view
context_name(const analysis::WorkloadContextKind context) noexcept {
    switch (context) {
...
incident_type_name(const analysis::IncidentType type) noexcept {
    ...
    case analysis::IncidentType::storage_io_retry:
        return "Storage I/O retry";
    }
    return "Unknown";
}

[[nodiscard]] constexpr std::string_view
explanation_basis_name(const analysis::SymptomExplanationBasis basis) noexcept {
    switch (basis) {
    ...
    case analysis::SymptomExplanationBasis::system_event_alignment:
        return "Aligned system event";
    }
    return "No aligned symptom evidence";
}

[[nodiscard]] constexpr analysis::SimilarIncidentSymptom
similar_symptom(const ui::IncidentCategory category) noexcept {
    switch (category) {
...
[[nodiscard]] constexpr analysis::SimilarIncidentFeedback
similar_feedback(const ui::IncidentFeedback feedback) noexcept {
    switch (feedback) {
...
[[nodiscard]] constexpr std::string_view
similar_symptom_name(const analysis::SimilarIncidentSymptom symptom) noexcept {
    switch (symptom) {
    case analysis::SimilarIncidentSymptom::unknown:
        return "Unknown";
    case analysis::SimilarIncidentSymptom::system_freeze:
        return "System freeze";
    case analysis::SimilarIncidentSymptom::game_stutter:
        return "Game stutter";
    case analysis::SimilarIncidentSymptom::application_slowdown_or_hang:
        return "Application slowdown or hang";
    case analysis::SimilarIncidentSymptom::network:
        return "Network issue";
    case analysis::SimilarIncidentSymptom::audio:
        return "Audio issue";
    }
    return "Unknown";
}

[[nodiscard]] std::string context_evidence_text(const analysis::WorkloadContextEvidence& evidence) {
    char detail[192]{};
    ...
    case analysis::ContextSignalKind::process_name_match:
        std::snprintf(detail, sizeof(detail), "%.0f bounded process-name match%s",
                      evidence.observed_value, evidence.observed_value == 1.0 ? "" : "es");
        break;
    ...
    std::snprintf(result, sizeof(result), "%s: %s (support contribution %.1f%%)",
                  context_name(evidence.context).data(), detail, evidence.contribution * 100.0);
    return result;
}

[[nodiscard]] constexpr std::string_view
direction_name(const analysis::AnomalyDirection direction) noexcept {
    switch (direction) {
    case analysis::AnomalyDirection::unchanged:
        return "unchanged";
    case analysis::AnomalyDirection::higher:
        return "higher";
    case analysis::AnomalyDirection::lower:
        return "lower";
    }
    return "unchanged";
}

[[nodiscard]] double display_value(const analysis::MetricKind metric, const double value) noexcept {
    switch (metric) {
    case analysis::MetricKind::system_cpu:
    case analysis::MetricKind::system_memory:
    case analysis::MetricKind::process_cpu:
...
[[nodiscard]] constexpr std::string_view display_unit(const analysis::MetricKind metric) noexcept {
    switch (metric) {
    case analysis::MetricKind::system_cpu:
    case analysis::MetricKind::system_memory:
    case analysis::MetricKind::process_cpu:
...
[[nodiscard]] const analysis::MetricAnomalyEvidence*
strongest_evidence(const std::vector<analysis::MetricAnomalyEvidence>& evidence) noexcept {
...
evidence_text(const std::vector<analysis::MetricAnomalyEvidence>& evidence,
    ...
    if (preferred_metric) {
        const auto found = std::find_if(evidence.begin(), evidence.end(), [&](const auto& item) {
    ...
    if (strongest == nullptr) {
        const auto insufficient =
            std::any_of(evidence.begin(), evidence.end(), [](const auto& item) {
                return item.availability == analysis::EvidenceAvailability::insufficient_baseline;
            });
    ...
    char text[320]{};
    const auto* scope =
        strongest->baseline_scope == analysis::BaselineScope::personalized_executable
            ? "personal executable"
            : "incident-local";
    std::snprintf(
        text, sizeof(text), "%s %s %s: %.2f%s vs median %.2f%s (P95 %.2f%s, |z| %.2f, n=%zu)",
        scope, metric_name(strongest->metric).data(), direction_name(strongest->direction).data(),
...
[[nodiscard]] std::string contributor_timing_text(const analysis::ContributorCandidate& candidate) {
    char text[384]{};
    if (candidate.temporal_relationship ==
        ...
        std::snprintf(text, sizeof(text),
                      ...
                      candidate.anomalous_duration_seconds, candidate.pre_marker_anomalous_samples,
    ...
    if (candidate.has_process_start_event) {
        ...
        std::snprintf(lifecycle, sizeof(lifecycle), "; recorded process start %+.1f s from marker",
                      seconds);
        result += lifecycle;
    }
    if (candidate.has_process_exit_event) {
        ...
        std::snprintf(lifecycle, sizeof(lifecycle), "; recorded process exit %+.1f s from marker",
                      seconds);
        result += lifecycle;
    }
    return result;
...
ui_temporal_relationship(const analysis::ContributorTemporalRelationship relationship) noexcept {
    switch (relationship) {
    ...
    case analysis::ContributorTemporalRelationship::marker_spanning_ambiguous:
        return ui::IncidentContributorRow::TemporalRelationship::marker_spanning_ambiguous;
...
[[nodiscard]] std::string
contributor_assessment_text(const analysis::ContributorCandidate& candidate) {
    if (candidate.temporal_relationship ==
...
[[nodiscard]] std::string
contributor_evidence_text(const analysis::ContributorCandidate& candidate) {
    char text[320]{};
    std::snprintf(text, sizeof(text),
                  "anomaly %.0f%% + timing %.0f%% + resource match %.0f%% + duration "
                  "%.0f%% + "
...
[[nodiscard]] RecurrenceAnalysisInput
recurrence_context_for(const ui::RecurringIncidentView& recurring, const std::int64_t incident_id) {
    ...
    for (const auto& group : recurring.groups) {
        const auto member =
            std::find_if(group.members.begin(), group.members.end(),
                         [incident_id](const auto& value) { return value.id == incident_id; });
        ...
        for (const auto& candidate : group.members) {
            input.feedback_history.push_back(analysis::SimilarIncidentFeedbackObservation{
                candidate.id, candidate.created_utc_milliseconds,
                similar_symptom(candidate.category), similar_feedback(candidate.user_feedback)});
        }
        return input;
    }
    const auto noise =
        std::find_if(recurring.noise.begin(), recurring.noise.end(),
                     [incident_id](const auto& value) { return value.id == incident_id; });
    if (noise != recurring.noise.end()) result.occurrence_count = 1U;
    return input;
}

[[nodiscard]] double seconds_from_event(const core::MonotonicTimePoint value,
...
struct AnalysisRun {
    ...
    std::vector<storage::StoredContributorFeedbackObservation> current_contributor_feedback{};
};

[[nodiscard]] constexpr analysis::ResourceKind
analysis_resource(const storage::ContributorFeedbackResource resource) noexcept {
    switch (resource) {
...
[[nodiscard]] constexpr analysis::ContributorTemporalRelationship analysis_temporal_relationship(
    ...
    case storage::ContributorFeedbackTemporalRelationship::marker_spanning_ambiguous:
...
[[nodiscard]] constexpr ui::IncidentContributorRow::Resource
ui_resource(const analysis::ResourceKind resource) noexcept {
    switch (resource) {
    case analysis::ResourceKind::cpu:
...
[[nodiscard]] AnalysisRun run_analysis(analysis::IIncidentAnalyzer* analyzer,
    ...
    if (analyzer == nullptr) {
        return {std::unexpected{analysis::AnalysisError{analysis::AnalysisErrorCode::internal_error,
                                                        "Analysis disabled"}},
                {},
                {}};
    }
    ...
    if (feedback_repository != nullptr) {
        ...
        if (!stored_feedback) {
            append_status("Feedback profile unavailable: " + stored_feedback.error().message);
        } else {
            observed_utc_milliseconds = stored_feedback->incident_utc_milliseconds;
            feedback_profile_revision = stored_feedback->profile_revision;
            feedback_profile_reset_after = stored_feedback->reset_after_utc_milliseconds;
            feedback_profile_rollback_available = stored_feedback->rollback_available;
            ...
            for (const auto& observation : stored_feedback->history) {
                feedback_history.push_back(analysis::FeedbackObservation{
                    ...
                    observation.automatic_resource, observation.automatic_signal,
                    observation.feedback == storage::IncidentUserFeedback::did_not_notice_problem
    ...
    if (observed_utc_milliseconds == 0) {
        observed_utc_milliseconds = recurrence_input.current_created_utc_milliseconds;
    }
    ...
    std::vector<analysis::ContributorFeedbackObservation> contributor_feedback_history;
    std::vector<storage::StoredContributorFeedbackObservation> current_contributor_feedback;
    if (feedback_repository != nullptr) {
        const auto stored_contributors = feedback_repository->contributor_feedback_context(
            incident_id, keys, storage::maximum_contributor_feedback_observations);
        if (!stored_contributors) {
            append_status("Contributor feedback unavailable: " +
                          stored_contributors.error().message);
        } else {
            if (observed_utc_milliseconds == 0) {
                observed_utc_milliseconds = stored_contributors->incident_utc_milliseconds;
            }
            current_contributor_feedback = stored_contributors->current;
            contributor_feedback_history.reserve(stored_contributors->history.size());
            for (const auto& observation : stored_contributors->history) {
                contributor_feedback_history.push_back(analysis::ContributorFeedbackObservation{
                    observation.incident_id, observation.incident_utc_milliseconds,
                    observation.feedback_updated_utc_milliseconds, observation.executable_key,
                    ...
                    analysis_temporal_relationship(observation.temporal_relationship)});
            }
        }
    }
    ...
    if (!analyzer->uses_personalized_history() || profile_repository == nullptr) {
        return {analyzer->analyze(
                    incident, analysis::IncidentAnalysisContext{incident_id,
                                                                observed_utc_milliseconds,
                                                                {},
                                                                recurrence_input.recurrence,
                                                                feedback_history,
                                                                feedback_profile_revision,
                ...
                std::move(profile_status), std::move(current_contributor_feedback)};
    }
    const auto stored_context = profile_repository->process_profile_context(incident_id, keys);
    if (!stored_context) {
        append_status("Personal history unavailable: " + stored_context.error().message);
        return {analyzer->analyze(
                    incident, analysis::IncidentAnalysisContext{incident_id,
                                                                observed_utc_milliseconds,
                                                                {},
    ...
    for (const auto& observation : stored_context->history) {
        history.push_back(analysis::ExecutableProfileObservation{
            observation.executable_key, observation.display_name, observation.incident_id,
            observation.observed_utc_milliseconds, observation.cpu_fraction,
            observation.working_set_bytes, observation.disk_read_bytes_per_second,
            observation.disk_write_bytes_per_second});
    }
    if (observed_utc_milliseconds == 0) {
        observed_utc_milliseconds = stored_context->incident_utc_milliseconds;
    }
    const analysis::IncidentAnalysisContext context{incident_id,
                                                    observed_utc_milliseconds,
                                                    history,
                                                    recurrence_input.recurrence,
                                                    feedback_history,
    ...
    if (const auto stored = profile_repository->store_process_profile_updates(incident_id, updates);
        !stored) {
...
[[nodiscard]] std::string diagnosis_evidence_text(const analysis::DiagnosisEvidenceLink& link,
    ...
    case analysis::DiagnosisEvidenceKind::contributor_correlation: {
        ...
        std::snprintf(text, sizeof(text),
                      ...
                      "contribution %.1f%%",
                      contributor.name.c_str(), contributor.identity.pid, link.source_score * 100.0,
                      detail.c_str(), link.confidence_contribution * 100.0);
        break;
    }
    case analysis::DiagnosisEvidenceKind::workload_context: {
        ...
        std::snprintf(text, sizeof(text),
                      "Probabilistic workload context %s %.1f%%; confidence "
                      "contribution %.1f%%",
                      context_name(probability.context).data(), probability.probability * 100.0,
    ...
    case analysis::DiagnosisEvidenceKind::recurring_pattern:
        std::snprintf(
            text, sizeof(text),
            "Automatic recurring pattern across %zu incidents, maximum pair "
            "distance "
            "%.3f, %zu shared characteristics; confidence contribution %.1f%%",
            analyzed.recurrence.occurrence_count, analyzed.recurrence.maximum_pair_distance,
            analyzed.recurrence.shared_characteristic_count, link.confidence_contribution * 100.0);
        break;
    case analysis::DiagnosisEvidenceKind::automatic_capture_trigger:
        std::snprintf(text, sizeof(text),
                      "Recorded automatic capture trigger score %.1f%% from %u "
                      "trigger%s; "
                      "confidence contribution %.1f%%",
                      link.source_score * 100.0, incident.header().window.automatic_trigger_count,
    ...
    case analysis::DiagnosisEvidenceKind::system_event: {
        ...
        const auto event_name =
            ...
            : event.kind == core::SystemEventKind::storage_io_retry ? "Disk I/O retry"
            : event.kind == core::SystemEventKind::dns_resolution_timeout
                ? "DNS Client resolution-timeout"
            : event.kind == core::SystemEventKind::application_crash ? "Application Error"
            : event.kind == core::SystemEventKind::application_hang  ? "Application Hang"
                                                                     : "normalized system";
        std::snprintf(text, sizeof(text),
                      "The system-event provider recorded %s event %u within five "
                      "seconds of the marker; confidence contribution %.1f%%",
                      event_name, event.native_event_id, link.confidence_contribution * 100.0);
        break;
    }
    }
    return text;
...
[[nodiscard]] ui::IncidentAnalysisView build_analysis_view(const AnalysisRun& run,
                                                           const core::IncidentSnapshot& incident) {
    ...
    view.status = analyzed->cold_start ? "Cold start: insufficient pre-incident system baseline"
                  : personalized_ready ? "Personalized executable ranking with aging history"
                  : personalized_cold
    ...
    const auto pressure = std::max_element(analyzed->resources.begin(), analyzed->resources.end(),
    ...
    if (pressure != analyzed->resources.end() && pressure->score > 0.0 &&
        ...
        view.pressure.evidence = evidence_text(pressure->evidence, pressure->pressure_metric);
    }
    view.diagnosis.available = analyzed->diagnosis.available;
    view.diagnosis.incident_type = std::string{incident_type_name(analyzed->diagnosis.type)};
    view.diagnosis.basis = std::string{explanation_basis_name(analyzed->diagnosis.basis)};
    view.diagnosis.confidence = std::string{confidence_name(analyzed->diagnosis.confidence)};
    view.diagnosis.calibrated_confidence = analyzed->diagnosis.calibrated_confidence;
    view.diagnosis.evidence_coverage = analyzed->diagnosis.evidence_coverage;
    view.diagnosis.correlated_evidence_penalty = analyzed->diagnosis.correlated_evidence_penalty;
    view.diagnosis.confidence_before_feedback = analyzed->diagnosis.confidence_before_feedback;
    view.diagnosis.feedback_multiplier = analyzed->diagnosis.feedback_multiplier;
    view.diagnosis.suppressed_by_feedback = analyzed->diagnosis.suppressed_by_feedback;
    view.diagnosis.pipeline_version = analyzed->provenance.pipeline_version;
    view.diagnosis.evidence_model_version = analyzed->provenance.evidence_model_version;
    view.diagnosis.configuration_fingerprint = analyzed->provenance.configuration_fingerprint;
    ...
    if (analyzed->diagnosis.primary_contributor_index &&
        *analyzed->diagnosis.primary_contributor_index < analyzed->contributors.size()) {
        const auto& contributor =
            analyzed->contributors[*analyzed->diagnosis.primary_contributor_index];
        view.diagnosis.primary_contributor =
            contributor.name + " (PID " + std::to_string(contributor.identity.pid) + ")";
    }
    view.diagnosis.evidence.reserve(analyzed->diagnosis.evidence.size());
    for (const auto& evidence : analyzed->diagnosis.evidence) {
        view.diagnosis.evidence.push_back(diagnosis_evidence_text(evidence, *analyzed, incident));
    }
    const auto& feedback = analyzed->feedback_calibration;
    view.feedback.applicable = feedback.state != analysis::FeedbackCalibrationState::not_applicable;
    view.feedback.ready = feedback.state == analysis::FeedbackCalibrationState::stable ||
                          feedback.state == analysis::FeedbackCalibrationState::suppressing;
    view.feedback.suppressing = feedback.state == analysis::FeedbackCalibrationState::suppressing;
    ...
    view.feedback.confirmed_problem_observations = feedback.confirmed_problem_observations;
    view.feedback.false_positive_observations = feedback.false_positive_observations;
    ...
    view.feedback.reset_after_utc_milliseconds = feedback.reset_after_utc_milliseconds;
    ...
    if (feedback.state == analysis::FeedbackCalibrationState::cold_start) {
        view.feedback.status =
            "Feedback profile cold start: " + std::to_string(feedback.matching_observations) +
        ...
        std::snprintf(status, sizeof(status),
                      "Feedback profile reduced automatic-trigger confidence "
                      "x%.3f: %zu of %zu matching triggers were not noticed",
                      feedback.confidence_multiplier, feedback.false_positive_observations,
    ...
    view.similar_incidents.ready = similar.state == analysis::SimilarIncidentEvidenceState::ready;
    view.similar_incidents.manually_excluded =
        similar.state == analysis::SimilarIncidentEvidenceState::manual_group_excluded;
    view.similar_incidents.symptom = std::string{similar_symptom_name(similar.symptom)};
    view.similar_incidents.observations_considered = similar.observations_considered;
    view.similar_incidents.answered_observations = similar.answered_observations;
    view.similar_incidents.confirmed_problem_observations = similar.confirmed_problem_observations;
    view.similar_incidents.false_positive_observations = similar.false_positive_observations;
    view.similar_incidents.categorized_confirmations = similar.categorized_confirmations;
    view.similar_incidents.matching_confirmations = similar.matching_confirmations;
    ...
    if (similar.state == analysis::SimilarIncidentEvidenceState::manual_group_excluded) {
        view.similar_incidents.status =
            "User-created recurrence groups are excluded from learned context";
    } else if (similar.state == analysis::SimilarIncidentEvidenceState::cold_start) {
        view.similar_incidents.status = "Confirmed similar-incident context is cold: " +
                                        std::to_string(similar.matching_confirmations) +
                                        " matching prior confirmations; at least 2 are required";
    } else if (similar.state == analysis::SimilarIncidentEvidenceState::conflicting) {
        char status[320]{};
        std::snprintf(status, sizeof(status),
                      "Prior feedback is conflicting: %.0f%% confirmed a problem "
                      "and %.0f%% of categorized confirmations agree on %s",
                      similar.problem_fraction * 100.0, similar.category_consensus * 100.0,
        ...
        char status[320]{};
        std::snprintf(status, sizeof(status),
                      "%zu of %zu categorized prior confirmations agree on %s; "
                      "%zu of %zu "
                      "answered similar incidents confirmed a noticed problem",
                      similar.matching_confirmations, similar.categorized_confirmations,
                      similar_symptom_name(similar.symptom).data(),
                      similar.confirmed_problem_observations, similar.answered_observations);
        view.similar_incidents.status = status;
    }
    ...
    for (const auto& item : analyzed->contributors) {
        ui::IncidentContributorRow row{
            item.name + " (PID " + std::to_string(item.identity.pid) + ")", item.score,
            contributor_assessment_text(item), contributor_timing_text(item),
            contributor_evidence_text(item)};
        row.executable_key = item.executable_key;
        row.resource = ui_resource(item.matched_resource);
        row.temporal_relationship = ui_temporal_relationship(item.temporal_relationship);
        row.score_before_feedback = item.score_before_feedback;
        row.feedback_multiplier = item.feedback_multiplier;
        row.feedback_matching_observations = item.feedback_matching_observations;
        row.feedback_confirmed_observations = item.feedback_confirmed_observations;
        row.feedback_rejected_observations = item.feedback_rejected_observations;
        const auto current =
            std::find_if(run.current_contributor_feedback.begin(),
        ...
        case analysis::ContributorFeedbackState::cold_start:
            row.feedback_status =
                "Attribution history cold: " + std::to_string(item.feedback_matching_observations) +
                " exact prior match(es); 4 required";
            break;
        case analysis::ContributorFeedbackState::conflicting:
            row.feedback_status = "Conflicting exact-match attribution "
                                  "retained without influence";
            break;
        case analysis::ContributorFeedbackState::stable:
            row.feedback_status = "Confirmed consensus inspected; post-marker "
        ...
        case analysis::ContributorFeedbackState::reduced: {
            char status[256]{};
            std::snprintf(status, sizeof(status),
                          "%s x%.3f from %zu confirmed / %zu rejected exact prior "
                          "attribution(s)",
                          item.feedback_state == analysis::ContributorFeedbackState::promoted
                              ? "Bounded promotion"
                              : "Bounded reduction",
                          item.feedback_multiplier, item.feedback_confirmed_observations,
    ...
    for (const auto& item : analyzed->resources) {
        ...
        append_context_adjustment(evidence, item.uncontextualized_score, item.context_multiplier);
    ...
    const auto process_count =
        (std::min)(analyzed->processes.size(), ui::incident_analysis_process_capacity);
    ...
    for (std::size_t index = 0U; index < process_count; ++index) {
        ...
        if (item.personalization == analysis::PersonalizationState::cold_start) {
            evidence = "Personal profile cold start (" +
                       std::to_string(item.personalized_observations) + " observations); " +
                       evidence;
        }
        ...
        view.processes.push_back(
            ui::IncidentAnomalyRow{item.name + " (PID " + std::to_string(item.identity.pid) + ")",
...
[[nodiscard]] constexpr std::string_view
feature_dimension_name(const analysis::IncidentFeatureDimension dimension) noexcept {
    switch (dimension) {
    case analysis::IncidentFeatureDimension::cpu_peak:
        return "CPU peak";
    case analysis::IncidentFeatureDimension::cpu_near_marker:
        return "CPU near marker";
    case analysis::IncidentFeatureDimension::memory_peak:
        return "memory peak";
    case analysis::IncidentFeatureDimension::memory_near_marker:
        return "memory near marker";
    case analysis::IncidentFeatureDimension::disk_peak:
        return "disk peak";
    case analysis::IncidentFeatureDimension::disk_near_marker:
        return "disk near marker";
    case analysis::IncidentFeatureDimension::network_peak:
        return "network peak";
    ...
    case analysis::IncidentFeatureDimension::duration:
...
[[nodiscard]] std::string shared_characteristics_text(const analysis::IncidentCluster& cluster) {
    ...
    for (const auto& characteristic : cluster.shared_characteristics) {
        ...
        std::snprintf(value, sizeof(value), "%s %.0f%% (%.0f%% support)",
                      ...
                      characteristic.median * 100.0, characteristic.support * 100.0);
        result += value;
    }
    return result;
}
...
ui::IncidentAnalysisView
analyze_incident(analysis::IIncidentAnalyzer* analyzer,
                 storage::IProcessProfileRepository* profile_repository,
                 storage::IFeedbackCalibrationRepository* feedback_repository,
                 const std::int64_t incident_id, const core::IncidentSnapshot& incident,
                 const ui::RecurringIncidentView& recurring) {
    const auto recurrence = recurrence_context_for(recurring, incident_id);
    return build_analysis_view(
        run_analysis(analyzer, profile_repository, feedback_repository, incident_id, incident,
                     recurrence),
        incident);
}

std::string shared_characteristics_text(const analysis::IncidentCluster& cluster) {
    return ::blackbox::app::shared_characteristics_text(cluster);
}
