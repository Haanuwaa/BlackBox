...
constexpr std::string_view privacy_readme =
    ...
    "diagnostics.ini contains bounded counters, feature/privacy switches, "
    "archive\n"
    "health totals, and product/platform versions. It deliberately excludes "
    "the\n"
    "incident archive, telemetry samples, process rows, executable names or "
    "paths,\n"
    "annotations, settings values, hotkeys, usernames, and absolute local "
    "paths.\n"
    "\n"
    "If crash-evidence.bin is present, the user explicitly chose to include "
    "it.\n"
...
[[nodiscard]] SupportBundleError error(const SupportBundleErrorCode code, std::string message) {
...
[[nodiscard]] bool write_file(const std::filesystem::path& path, const std::string_view contents) {
...
[[nodiscard]] std::expected<std::uint64_t, SupportBundleError>
fingerprint_file(const std::filesystem::path& path) {
    std::ifstream input{path, std::ios::binary};
    if (!input) {
        return std::unexpected{
            error(SupportBundleErrorCode::cannot_write, "support artifact cannot be read back")};
    }
    ...
    while (input) {
        ...
        if (count < 0) {
            return std::unexpected{
                error(SupportBundleErrorCode::cannot_write, "support artifact read-back failed")};
        }
    ...
    if (!input.eof() || total == 0U) {
        return std::unexpected{
            error(SupportBundleErrorCode::cannot_write, "support artifact is empty or unreadable")};
    }
    return hash;
}

void append_boolean(std::ostringstream& output, const std::string_view name, const bool value) {
    output << name << '=' << (value ? 1 : 0) << '\n';
}

template <typename Integer>
void append_integer(std::ostringstream& output, const std::string_view name, const Integer value) {
    output << name << '=' << value << '\n';
}
...
[[nodiscard]] std::string diagnostics_text(const SupportDiagnostics& value) {
    ...
    append_boolean(output, "automatic_detection_enabled", value.automatic_detection_enabled);
    append_boolean(output, "process_path_collection_enabled",
                   value.process_path_collection_enabled);
    append_boolean(output, "foreground_identity_enabled", value.foreground_identity_enabled);
    append_boolean(output, "process_lifecycle_enabled", value.process_lifecycle_enabled);
    append_boolean(output, "power_and_device_events_enabled",
                   value.power_and_device_events_enabled);
    append_boolean(output, "audio_device_events_enabled", value.audio_device_events_enabled);
    append_boolean(output, "system_event_evidence_enabled", value.system_event_evidence_enabled);
    ...
    append_integer(output, "collector_worker_failures", value.collector_worker_failures);
    append_integer(output, "incident_captures_started", value.incident_captures_started);
    append_integer(output, "incidents_completed", value.incidents_completed);
    append_integer(output, "incident_snapshot_failures", value.incident_snapshot_failures);
    append_integer(output, "incident_queue_rejections", value.incident_queue_rejections);
    append_integer(output, "automatic_detector_triggers", value.automatic_detector_triggers);
    ...
    append_integer(output, "process_lifecycle_observations", value.process_lifecycle_observations);
...
[[nodiscard]] std::expected<void, SupportBundleError>
validate_crash_source(const std::filesystem::path& path) {
...
std::expected<SupportBundleResult, SupportBundleError>
    ...
    try {
        if (request.destination.empty() || !request.destination.is_absolute() ||
            ...
            return std::unexpected{error(SupportBundleErrorCode::invalid_request,
                                         "support bundle requires a new absolute "
                                         "destination and safe version fields")};
        }
        ...
        if (request.consented_crash_evidence.has_value() !=
            ...
            return std::unexpected{error(SupportBundleErrorCode::invalid_request,
                                         "raw crash evidence inclusion requires "
                                         "matching explicit disclosure consent")};
        }
        if (request.consented_crash_evidence) {
            if (const auto valid = validate_crash_source(*request.consented_crash_evidence);
                !valid) {
        ...
        const auto fail = [](SupportBundleError failure) {
            // A failed partial directory is preserved for inspection and can
            // never be mistaken for a published support bundle.
        ...
        for (std::filesystem::directory_iterator iterator{staging, filesystem_error}, end;
            ...
            if (filesystem_error || size == 0U || size > maximum_support_crash_evidence_bytes ||
        ...
        return SupportBundleResult{request.destination, static_cast<std::uint64_t>(expected.size()),
                                   bytes, included_crash_evidence};
    } catch (const std::exception& exception) {
        return std::unexpected{error(SupportBundleErrorCode::cannot_write, exception.what())};
    } catch (...) {
        return std::unexpected{
            error(SupportBundleErrorCode::cannot_write, "unknown support bundle failure")};
    }
}
...
std::shared_ptr<const SupportBundleServiceSnapshot> SupportBundleService::snapshot() const {
    const std::scoped_lock lock{mutex_};
    return snapshot_;
}
...
void SupportBundleService::run(const std::stop_token token) noexcept {
    while (!token.stop_requested()) {
        SupportBundleRequest request;
        {
            ...
            static_cast<void>(
                available_.wait(lock, token, [this] { return pending_.has_value(); }));
        ...
        if (result) {
            publish(false, "Support bundle created locally with " + std::to_string(result->files) +
                               " files");
        } else {
