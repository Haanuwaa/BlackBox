...
void IncidentViewerService::handle_detail(const Job& job) {
    ...
    if (analyzer_ == nullptr) {
        ...
        const auto recurrence = recurrence_context_for(snapshot()->recurring, job.incident_id);
        const auto analysis_run = run_analysis(analyzer_, profile_repository_, feedback_repository_,
                                               job.incident_id, **incident, recurrence);
...
void IncidentViewerService::handle_recurring() {
    ...
    if (analyzer_ != nullptr && loaded_incident_ != nullptr && content.detail &&
        ...
        const auto analysis_started = std::chrono::steady_clock::now();
        const auto recurrence = recurrence_context_for(content.recurring, loaded_incident_id_);
        const auto analysis_run = run_analysis(analyzer_, profile_repository_, feedback_repository_,
                                               loaded_incident_id_, *loaded_incident_, recurrence);
