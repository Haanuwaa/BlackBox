...
void Application::write_diagnostic_report() noexcept {
    ...
    report.capture_interval_seconds = static_cast<std::uint64_t>(
        diagnostic_options_.capture_interval.count());
    ...
    if (system_event_collector_ != nullptr) {
        ...
        // The wall-clock qualification artifact accounts only for native
        // provider families. Opt-in lifecycle evidence has its own collector
        // diagnostics and stays outside that stable soak contract.
        ...
        report.windows_update_events_recorded = values.events_by_source.windows_update;
