...
[[nodiscard]] std::optional<std::string> environment_value(const char* name) {
    ...
    if (getenv_s(&required, value.data(), required, name) != 0) {
        return std::nullopt;
    }
    return value;
#else
...
[[nodiscard]] ProductSettingsError settings_error(
    const ProductSettingsErrorCode code, std::string message) {
...
std::filesystem::path default_product_settings_path() {
    if (const auto override_path = environment_value("BLACKBOX_PRODUCT_SETTINGS_PATH")) {
    ...
    if (const auto home = environment_value("HOME")) {
        return std::filesystem::path{*home} / ".config" / "blackbox" /
...
std::expected<ProductSettings, ProductSettingsError> validate_product_settings(
    ProductSettings settings) noexcept {
    try {
        ...
        if (key < static_cast<std::uint8_t>(platform::HotkeyKey::f1) ||
            ...
            (!settings.incident_hotkey.control && !settings.incident_hotkey.shift &&
             !settings.incident_hotkey.alt && !settings.incident_hotkey.windows)) {
            return std::unexpected{settings_error(
                ProductSettingsErrorCode::invalid_configuration,
        ...
        if (settings.automatic_detection_enabled && !settings.detect_cpu &&
            !settings.detect_memory && !settings.detect_disk &&
            !settings.detect_network) {
            return std::unexpected{settings_error(
                ProductSettingsErrorCode::invalid_configuration,
        ...
        if (!settings.archive_path.is_absolute() ||
            !settings.archive_path.has_filename() || !safe_field(archive)) {
            return std::unexpected{settings_error(
                ProductSettingsErrorCode::invalid_configuration,
                "archive path must be an absolute file path of at most 1024 bytes")};
        }
        ...
        return std::unexpected{settings_error(
            ProductSettingsErrorCode::invalid_configuration, exception.what())};
    } catch (...) {
        return std::unexpected{settings_error(
            ProductSettingsErrorCode::invalid_configuration,
...
std::expected<ProductSettings, ProductSettingsError> parse_product_settings_text(
    ...
    try {
        ...
        constexpr std::array<std::string_view, 22U> required{
            "format", "hotkey_key", "hotkey_control", "hotkey_shift", "hotkey_alt",
            "hotkey_windows", "automatic_detection", "detector_sensitivity",
            "detect_cpu", "detect_memory", "detect_disk", "detect_network",
        ...
        if (!fields.contains("format") ||
            !parse_integer(fields["format"], format) ||
        ...
        if (!parse_integer(fields["format"], format) ||
            ...
            !parse_boolean(fields["hotkey_windows"], settings.incident_hotkey.windows) ||
            !parse_boolean(fields["automatic_detection"],
            ...
            !parse_boolean(fields["onboarding_completed"],
        ...
        if (!parse_boolean(fields["record_foreground_application"],
            ...
            !parse_boolean(fields["record_process_lifecycle"],
                           settings.record_process_lifecycle) ||
        ...
        settings.archive_path = std::filesystem::path{std::u8string{
            reinterpret_cast<const char8_t*>(archive_text.data()),
            archive_text.size()}};
        return validate_product_settings(std::move(settings));
    } catch (const std::exception& exception) {
        return std::unexpected{settings_error(
            ProductSettingsErrorCode::invalid_format, exception.what())};
    } catch (...) {
        return std::unexpected{settings_error(
            ProductSettingsErrorCode::invalid_format,
...
std::expected<ProductSettings, ProductSettingsError> load_product_settings(
    ...
    try {
        ...
        if (!stream) {
            return std::unexpected{settings_error(
                ProductSettingsErrorCode::cannot_read,
        ...
        return std::unexpected{settings_error(
            ProductSettingsErrorCode::cannot_read, exception.what())};
    } catch (...) {
        return std::unexpected{settings_error(
            ProductSettingsErrorCode::cannot_read,
...
std::expected<void, ProductSettingsError> save_product_settings(
    const std::filesystem::path& path, const ProductSettings& settings) noexcept {
    try {
        ...
        contents << "format=" << product_settings_format_version << '\n'
                 ...
                 << "hotkey_windows=" << settings.incident_hotkey.windows << '\n'
                 << "automatic_detection=" << settings.automatic_detection_enabled << '\n'
                 << "detector_sensitivity="
                 << static_cast<unsigned>(settings.detector_sensitivity) << '\n'
        ...
        backup += ".bak";
        {
            ...
            if (!stream || !(stream << contents.str()) || !stream.flush()) {
                return std::unexpected{settings_error(
                    ProductSettingsErrorCode::cannot_write,
        ...
        return std::unexpected{settings_error(
            ProductSettingsErrorCode::cannot_write, exception.what())};
    } catch (...) {
        return std::unexpected{settings_error(
            ProductSettingsErrorCode::cannot_write,
...
telemetry::AutomaticDetectorConfiguration detector_configuration(
...
telemetry::EventProviderConfiguration event_provider_configuration(
    const ProductSettings& settings) noexcept {
