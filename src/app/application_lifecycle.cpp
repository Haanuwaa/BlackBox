...
void Application::write_diagnostic_report() noexcept {
    ...
    if (const auto* video_driver = SDL_GetCurrentVideoDriver(); video_driver != nullptr) {
    ...
    if (collector_ != nullptr) {
        ...
        const auto scheduling_event_count =
            std::min(scheduling.events.size(), wall_clock_scheduling_drop_event_capacity);
        report.scheduling_drop_events.reserve(scheduling_event_count);
        report.scheduling_drop_event_overflow = scheduling.overflow;
        for (std::size_t index = 0U; index < scheduling_event_count; ++index) {
            const auto& source = scheduling.events[index];
            const auto utc =
                diagnostic_utc_anchor_ + (source.observed_at - diagnostic_monotonic_anchor_);
            const auto utc_nanoseconds =
        ...
        report.process_ring_overwritten_samples = values.process_ring.overwritten_samples;
        report.process_ring_discarded_samples = values.process_ring.discarded_samples;
        report.captures_started = values.incident_capture.captures_started;
        report.capture_requests_merged = values.incident_capture.capture_requests_merged;
        ...
        report.automatic_detector_triggers = values.automatic_detector.triggers_emitted;
    ...
    if (system_event_collector_ != nullptr) {
        ...
        report.system_events_recorded = values.events_recorded - values.external_events_recorded;
        ...
        report.service_events_recorded = values.events_by_source.service_manager;
        report.security_events_recorded = values.events_by_source.security;
        report.update_events_recorded = values.events_by_source.update;
    ...
    if (incident_archive_ != nullptr) {
        ...
        if (count) report.archive_incidents = *count;
        if (size) report.archive_size_bytes = *size;
        if (schema) report.archive_schema_version = *schema;
        report.archive_healthy = count.has_value() && size.has_value() && schema.has_value() &&
