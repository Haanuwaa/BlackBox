...
bool matches_macos_hotkey(const HotkeyCombination combination,
                          const std::uint32_t function_character, const bool control,
                          const bool shift, const bool option, const bool command) noexcept {
    const auto expected = macos_function_character(combination.key);
    return expected != 0U && function_character == expected && combination.control == control &&
           combination.shift == shift && combination.alt == option &&
           combination.system_modifier == command;
}
...
struct MacosGlobalHotkeyManager::NativeState {
    [[nodiscard]] bool matches(NSEvent* event) const noexcept {
        ...
        return matches_macos_hotkey(
            ...
            (flags & NSEventModifierFlagControl) != 0U, (flags & NSEventModifierFlagShift) != 0U,
            (flags & NSEventModifierFlagOption) != 0U, (flags & NSEventModifierFlagCommand) != 0U);
    }
...
MacosGlobalHotkeyManager::MacosGlobalHotkeyManager() : native_{std::make_unique<NativeState>()} {}

MacosGlobalHotkeyManager::~MacosGlobalHotkeyManager() { unregister_hotkey(); }

HotkeyRegistrationResult
MacosGlobalHotkeyManager::register_hotkey(const HotkeyCombination combination,
                                          HotkeyCallback callback) {
    unregister_hotkey();
    ...
    @autoreleasepool {
        ...
        native_->global_monitor = [NSEvent addGlobalMonitorForEventsMatchingMask:NSEventMaskKeyDown
                                                                         handler:^(NSEvent* event) {
                                                                           state->activate(event);
                                                                         }];
        native_->local_monitor =
...
bool MacosGlobalHotkeyManager::registered() const noexcept {
    return native_ != nullptr && native_->global_monitor != nil && native_->local_monitor != nil;
}
