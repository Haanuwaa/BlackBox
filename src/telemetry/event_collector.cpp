...
void record_source(EventSourceCounts& counts,
                   const core::SystemEventSource source) noexcept {
    switch (source) {
    case core::SystemEventSource::power: ++counts.power; break;
    case core::SystemEventSource::device: ++counts.device; break;
    case core::SystemEventSource::audio: ++counts.audio; break;
    case core::SystemEventSource::service_control_manager:
        ++counts.service_control_manager;
        break;
    case core::SystemEventSource::defender: ++counts.defender; break;
    case core::SystemEventSource::windows_update: ++counts.windows_update; break;
    case core::SystemEventSource::application: ++counts.application; break;
    case core::SystemEventSource::network: ++counts.network; break;
    case core::SystemEventSource::graphics: ++counts.graphics; break;
    case core::SystemEventSource::storage: ++counts.storage; break;
    case core::SystemEventSource::process: ++counts.process; break;
    }
}
...
SystemEventCollector::SystemEventCollector(
    ...
    : provider_{provider},
      ...
      batch_{std::make_unique<
          std::array<core::SystemEvent, maximum_events_per_poll>>()} {
...
core::RecorderSnapshot<core::SystemEvent> SystemEventCollector::snapshot(
    const std::size_t maximum_events) const {
    return recorder_.snapshot(maximum_events);
}

bool SystemEventCollector::record_external_event(
    const core::SystemEvent& event) noexcept {
    const bool lifecycle_kind =
        ...
        while (!stop_token.stop_requested()) {
            {
                ...
                static_cast<void>(wake_condition_.wait_until(
                    lock, stop_token, next_poll, [] { return false; }));
            }
            ...
            poll_timing_.record(std::max(core::MonotonicClock::duration::zero(),
                                         clock_.now() - started));
            {
                const std::scoped_lock lock{diagnostics_mutex_};
                ...
                diagnostics_.events_by_source.service_control_manager +=
                    source_counts.service_control_manager;
                diagnostics_.events_by_source.defender += source_counts.defender;
                diagnostics_.events_by_source.windows_update += source_counts.windows_update;
