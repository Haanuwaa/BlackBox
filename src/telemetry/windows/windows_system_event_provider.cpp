...
file_time_to_unix_milliseconds(const std::uint64_t file_time) noexcept {
...
std::optional<core::SystemEventKind>
normalized_windows_event_kind(const core::SystemEventSource source,
    ...
    case core::SystemEventSource::security:
        if (native_event_id == 1000U) return core::SystemEventKind::security_scan_started;
        if (native_event_id == 1001U) return core::SystemEventKind::security_scan_completed;
        if (native_event_id == 1116U) return core::SystemEventKind::security_threat_detected;
        if (native_event_id >= 1117U && native_event_id <= 1119U)
            return core::SystemEventKind::security_action;
        if (native_event_id == 5007U) return core::SystemEventKind::security_configuration_changed;
        return std::nullopt;
    case core::SystemEventSource::update:
        if (native_event_id == 19U) return core::SystemEventKind::update_succeeded;
        if (native_event_id == 20U || native_event_id == 25U || native_event_id == 31U ||
            native_event_id == 34U)
            return core::SystemEventKind::update_failed;
        if (native_event_id == 41U || native_event_id == 43U || native_event_id == 44U)
            return core::SystemEventKind::update_activity_started;
        return std::nullopt;
    case core::SystemEventSource::application:
        if (native_event_id == 1000U) return core::SystemEventKind::application_crash;
        if (native_event_id == 1002U) return core::SystemEventKind::application_hang;
        return std::nullopt;
    ...
    case core::SystemEventSource::storage:
        return native_event_id == 153U ? std::optional{core::SystemEventKind::storage_io_retry}
                                       : std::nullopt;
...
struct WindowsSystemEventProvider::NativeState {
    ...
    public:
        ...
        HRESULT STDMETHODCALLTYPE OnDeviceStateChanged(LPCWSTR, DWORD state) override {
            owner_.push(make_audio(core::SystemEventKind::audio_endpoint_state_changed, state));
            return S_OK;
        }
        ...
        HRESULT STDMETHODCALLTYPE OnPropertyValueChanged(LPCWSTR, const PROPERTYKEY) override {
            // Property changes are both noisy and semantically weak; endpoint
            // state/default callbacks cover the user-visible transitions
            // without retaining identifiers.
            return S_OK;
        }

    private:
        [[nodiscard]] static core::SystemEvent
        make_audio(const core::SystemEventKind kind, const std::uint32_t detail = 0U) noexcept {
    ...
    [[nodiscard]] static DWORD CALLBACK device_callback(HCMNOTIFICATION, PVOID context,
    ...
    [[nodiscard]] static ULONG CALLBACK power_callback(PVOID context, ULONG type, PVOID) noexcept {
        auto* owner = static_cast<NativeState*>(context);
        if (owner == nullptr) return ERROR_SUCCESS;
        core::SystemEvent event{};
    ...
    [[nodiscard]] static DWORD WINAPI event_log_callback(EVT_SUBSCRIBE_NOTIFY_ACTION action,
                                                         PVOID context,
                                                         EVT_HANDLE event_handle) noexcept {
        ...
        const auto context_handle = EvtCreateRenderContext(static_cast<DWORD>(std::size(paths)),
                                                           paths, EvtRenderContextValues);
        ...
        DWORD count{};
        const auto rendered =
            EvtRender(context_handle, event_handle, EvtRenderEventValues,
        ...
        const auto event_id = values[0].Type == EvtVarTypeUInt16
                              ...
                              : values[0].Type == EvtVarTypeUInt32 ? values[0].UInt32Val
                                                                   : 0U;
        const auto level =
            static_cast<std::uint8_t>(values[1].Type == EvtVarTypeByte ? values[1].ByteVal : 4U);
        core::SystemEvent event{};
        ...
        if (values[2].Type == EvtVarTypeFileTime) {
            event.source_utc_milliseconds = file_time_to_unix_milliseconds(values[2].FileTimeVal);
    ...
    [[nodiscard]] bool map_event_kind(core::SystemEvent& event) const noexcept {
        const auto kind = normalized_windows_event_kind(event.source, event.native_event_id);
        if (!kind) return false;
        event.kind = *kind;
        return true;
    }

    [[nodiscard]] bool subscribe(const std::size_t index, LPCWSTR channel, LPCWSTR query,
                                 const core::SystemEventSource source) noexcept {
        contexts[index] = SubscriptionContext{this, source};
        subscriptions[index] =
            EvtSubscribe(nullptr, nullptr, channel, query, nullptr, &contexts[index],
                         event_log_callback, EvtSubscribeToFutureEvents);
        return subscriptions[index] != nullptr;
    }

    [[nodiscard]] EventProviderStatus start(const EventProviderConfiguration& requested) noexcept {
        {
            const std::scoped_lock lock{mutex};
            head = 0U;
            size = 0U;
        ...
        if (configuration.device_events) {
            ...
            if (CM_Register_Notification(&filter, this, device_callback, &device_registration) ==
                CR_SUCCESS) {
        ...
        if (configuration.power_events) {
            ...
            if (PowerRegisterSuspendResumeNotification(DEVICE_NOTIFY_CALLBACK, &parameters,
        ...
        if (configuration.audio_device_events) {
            ...
            if (com_available &&
                ...
                if (audio_notifications != nullptr &&
                    SUCCEEDED(audio_enumerator->RegisterEndpointNotificationCallback(
                        audio_notifications))) {
        ...
        if (configuration.service_events) {
            ...
            if (subscribe(subscription_index++, L"System",
                          ...
                          L"(EventID=7031 or EventID=7034 or EventID=7036 or "
                          L"EventID=7040)]]",
                          core::SystemEventSource::service_manager)) {
                capabilities.service_events = true;
                ++active_count;
            }
        }
        if (configuration.security_events) {
            ++requested_count;
            if (subscribe(subscription_index++, L"Microsoft-Windows-Windows Defender/Operational",
                          L"*[System[EventID=1000 or EventID=1001 or EventID=1116 or "
                          L"EventID=1117 or EventID=1118 or EventID=1119 or "
                          L"EventID=5007]]",
                          core::SystemEventSource::security)) {
                capabilities.security_events = true;
                ++active_count;
            }
        }
        if (configuration.update_events) {
            ++requested_count;
            if (subscribe(subscription_index++,
                          L"Microsoft-Windows-WindowsUpdateClient/Operational",
                          L"*[System[EventID=19 or EventID=20 or EventID=25 or "
                          L"EventID=31 or "
                          L"EventID=34 or EventID=41 or EventID=43 or EventID=44]]",
                          core::SystemEventSource::update)) {
                capabilities.update_events = true;
                ++active_count;
            }
        }
        if (configuration.application_events) {
            ...
            if (subscribe(subscription_index++, L"Application",
                          L"*[System[(Provider[@Name='Application Error'] and "
                          L"EventID=1000) or "
        ...
        if (configuration.network_events) {
            ++requested_count;
            if (subscribe(subscription_index++, L"System",
                          L"*[System[Provider[@Name='Microsoft-Windows-DNS-"
                          L"Client'] and "
                          L"EventID=1014]]",
    ...
    void stop() noexcept {
        ...
        if (audio_enumerator != nullptr && audio_notifications != nullptr) {
            static_cast<void>(
                audio_enumerator->UnregisterEndpointNotificationCallback(audio_notifications));
        }
        if (audio_notifications != nullptr) {
...
WindowsSystemEventProvider::~WindowsSystemEventProvider() { stop(); }

EventProviderStatus
WindowsSystemEventProvider::start(const EventProviderConfiguration& configuration) noexcept {
    return state_ != nullptr ? state_->start(configuration)
                             : EventProviderStatus::temporarily_failed;
}

EventProviderPollResult
WindowsSystemEventProvider::poll(const core::MonotonicTimePoint observed_at,
                                 const std::span<core::SystemEvent> destination) noexcept {
    if (state_ == nullptr) {
    ...
    const auto requested = static_cast<std::size_t>(configuration.power_events) +
                           ...
                           static_cast<std::size_t>(configuration.security_events) +
                           static_cast<std::size_t>(configuration.update_events) +
    ...
    const auto active = static_cast<std::size_t>(capabilities.power_events) +
                        ...
                        static_cast<std::size_t>(capabilities.security_events) +
                        static_cast<std::size_t>(capabilities.update_events) +
    ...
    const auto status = requested == 0U || active == requested ? EventProviderStatus::complete
...
EventProviderCapabilities WindowsSystemEventProvider::capabilities() const noexcept {
    ...
    return state_ == nullptr ? EventProviderCapabilities{}
                             : EventProviderCapabilities{true, true, true, true, true,
