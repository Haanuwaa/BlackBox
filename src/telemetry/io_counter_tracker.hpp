...
struct IoEntityCounters {
    ...
    friend constexpr bool operator==(const IoEntityCounters&,
                                     const IoEntityCounters&) = default;
};
...
// Produces monotonic aggregate counters from a changing set of native
// entities. A newly observed entity establishes a baseline, removal contributes
// no negative delta, and a reset invalidates only that channel for one sample.
// The fixed capacity keeps provider hot paths allocation-free.
...
public:
    [[nodiscard]] IoAggregateCounters update(
        ...
        if (!valid_input(counters)) return unavailable();

        for (auto& slot : slots_) {
            slot.seen = false;
            if (slot.active && !contains(counters, slot.identity)) slot.active = false;
        }
        ...
        for (const auto& counter : counters) {
            ...
            if (slot == nullptr) {
                slot = find_free();
                if (slot == nullptr) return unavailable();
        ...
        for (auto& slot : slots_) {
            if (slot.active && !slot.seen) slot.active = false;
        }
        return IoAggregateCounters{make_value(first_total_, first_status),
...
private:
    ...
    [[nodiscard]] static constexpr IoAggregateCounters unavailable() noexcept {
        return {MetricValue<ByteCount>::unavailable(
    ...
    [[nodiscard]] static constexpr MetricValue<ByteCount> make_value(
        const std::uint64_t value, const MetricStatus status) noexcept {
    ...
    [[nodiscard]] static bool valid_input(
        ...
        for (std::size_t left = 0U; left < counters.size(); ++left) {
            for (std::size_t right = left + 1U; right < counters.size(); ++right) {
                if (counters[left].identity == counters[right].identity) return false;
            }
        }
        return true;
    }
    ...
    [[nodiscard]] static bool contains(
        ...
        for (const auto& counter : counters) {
            if (counter.identity == identity) return true;
        }
        return false;
    }
    ...
    [[nodiscard]] Slot* find_active(const std::uint64_t identity) noexcept {
        for (auto& slot : slots_) {
            if (slot.active && slot.identity == identity) return &slot;
        }
        return nullptr;
    }
