...
[[nodiscard]] MetricValue<Ratio> normalize_cpu_usage(
...
[[nodiscard]] MetricValue<BytesPerSecond> normalize_byte_rate(
    const MetricValue<ByteCount>& previous,
    const MetricValue<ByteCount>& current,
    std::chrono::steady_clock::duration elapsed) noexcept;

[[nodiscard]] MetricValue<std::uint64_t> normalize_counter_delta(
    const MetricValue<std::uint64_t>& previous,
    const MetricValue<std::uint64_t>& current) noexcept;

[[nodiscard]] MetricValue<Ratio> normalize_tcp_retransmit_fraction(
...
[[nodiscard]] MetricValue<ByteCount> normalize_memory_used(
    const MetricValue<ByteCount>& total,
    const MetricValue<ByteCount>& available) noexcept;

[[nodiscard]] MetricValue<Ratio> normalize_memory_usage(
    const MetricValue<ByteCount>& total,
    const MetricValue<ByteCount>& available) noexcept;
