...
[[nodiscard]] StorageError database_error(sqlite3* database,
                                          ...
                                          const int override_code = SQLITE_OK) {
...
std::expected<void, StorageError> SqliteIncidentArchive::backup_to(
    ...
    try {
        ...
        auto* backup = sqlite3_backup_init(target, "main", native_->database, "main");
        if (backup == nullptr) {
        ...
        return std::unexpected{StorageError{StorageErrorCode::io, 0,
                                             exception.what()}};
    } catch (...) {
        return std::unexpected{simple_error(StorageErrorCode::io,
                                             "unknown archive backup failure")};
    }
}
...
std::expected<void, StorageError> SqliteIncidentArchive::restore_from(
    ...
    try {
        if (source.empty() || safety_backup.empty() ||
            ...
            return std::unexpected{simple_error(
                StorageErrorCode::invalid_data,
                "restore requires an existing source and a new distinct safety-backup file")};
        }
        const std::scoped_lock lock{native_->mutex};
        if (native_->database == nullptr) {
            return std::unexpected{simple_error(StorageErrorCode::not_open,
        ...
        const auto integrity = scalar_text(input, "PRAGMA integrity_check",
                                           "verify restore source");
        const auto version = scalar_int64(input, "PRAGMA user_version",
                                          "read restore schema version");
        const auto direct_schema = version && *version == current_schema_version
                                       ? validate_direct_schema_v1(input)
                                       : std::expected<void, StorageError>{
                                             std::unexpected{simple_error(
                                                 StorageErrorCode::invalid_schema,
        ...
        sqlite3* safety{};
#if defined(_WIN32)
        const auto safety_opened = sqlite3_open16(safety_backup.native().c_str(), &safety);
#else
        const auto safety_opened = sqlite3_open(safety_backup.string().c_str(), &safety);
#endif
        if (safety_opened != SQLITE_OK) {
            auto error = database_error(safety, "open restore safety backup", safety_opened);
            if (safety != nullptr) sqlite3_close(safety);
            sqlite3_close(input);
            return std::unexpected{std::move(error)};
        }
        auto* preserve = sqlite3_backup_init(safety, "main", native_->database, "main");
        const auto preserve_step = preserve == nullptr
            ? SQLITE_ERROR : sqlite3_backup_step(preserve, -1);
        const auto preserve_finish = preserve == nullptr
            ? SQLITE_ERROR : sqlite3_backup_finish(preserve);
        if (preserve == nullptr || preserve_step != SQLITE_DONE ||
            ...
            return std::unexpected{simple_error(StorageErrorCode::io,
                                                 "could not create restore safety backup")};
        }
        sqlite3_close(safety);
        auto* restore = sqlite3_backup_init(native_->database, "main", input, "main");
        if (restore == nullptr) {
            sqlite3_close(input);
            return std::unexpected{database_error(native_->database,
                                                   "initialize archive restore")};
        }
        const auto stepped = sqlite3_backup_step(restore, -1);
        const auto finished = sqlite3_backup_finish(restore);
        sqlite3_close(input);
        if ((stepped != SQLITE_DONE && stepped != SQLITE_OK) || finished != SQLITE_OK) {
            return std::unexpected{simple_error(StorageErrorCode::io,
                                                 "archive restore did not complete")};
        }
        if (auto limited = configure_size_limit(native_->database,
                                                configuration_.maximum_bytes);
            !limited) return std::unexpected{limited.error()};
        return {};
    } catch (const std::exception& exception) {
        return std::unexpected{StorageError{StorageErrorCode::io, 0,
                                             exception.what()}};
        ...
        return std::unexpected{simple_error(StorageErrorCode::io,
                                             "unknown archive restore failure")};
    }
}
