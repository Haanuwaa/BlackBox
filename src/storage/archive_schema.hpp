...
CREATE TABLE system_quality_samples (
 ...
 disk_service_concurrency_status INTEGER NOT NULL CHECK(disk_service_concurrency_status BETWEEN 0 AND 3),
 disk_service_concurrency REAL CHECK(disk_service_concurrency_status<>0 OR disk_service_concurrency IS NOT NULL),
...
R"sql(CREATE TABLE system_pressure_samples (
 ...
 compressed_memory_status INTEGER NOT NULL CHECK(compressed_memory_status BETWEEN 0 AND 3),
 compressed_memory_bytes BLOB CHECK(compressed_memory_status<>0 OR (compressed_memory_bytes IS NOT NULL AND length(compressed_memory_bytes)=8)),
 page_out_status INTEGER NOT NULL CHECK(page_out_status BETWEEN 0 AND 3),
 page_out_bps REAL CHECK(page_out_status<>0 OR page_out_bps IS NOT NULL),
 swap_in_status INTEGER NOT NULL CHECK(swap_in_status BETWEEN 0 AND 3),
 swap_in_bps REAL CHECK(swap_in_status<>0 OR swap_in_bps IS NOT NULL),
 swap_out_status INTEGER NOT NULL CHECK(swap_out_status BETWEEN 0 AND 3),
 swap_out_bps REAL CHECK(swap_out_status<>0 OR swap_out_bps IS NOT NULL),
 compression_status INTEGER NOT NULL CHECK(compression_status BETWEEN 0 AND 3),
 compression_bps REAL CHECK(compression_status<>0 OR compression_bps IS NOT NULL),
 decompression_status INTEGER NOT NULL CHECK(decompression_status BETWEEN 0 AND 3),
 decompression_bps REAL CHECK(decompression_status<>0 OR decompression_bps IS NOT NULL),
 scheduler_delay_status INTEGER NOT NULL CHECK(scheduler_delay_status BETWEEN 0 AND 3),
 scheduler_delay_seconds REAL CHECK(scheduler_delay_status<>0 OR scheduler_delay_seconds IS NOT NULL),
 logical_processors_status INTEGER NOT NULL CHECK(logical_processors_status BETWEEN 0 AND 3),
 logical_processors INTEGER CHECK(logical_processors_status<>0 OR logical_processors>0),
 physical_processors_status INTEGER NOT NULL CHECK(physical_processors_status BETWEEN 0 AND 3),
 physical_processors INTEGER CHECK(physical_processors_status<>0 OR physical_processors>0),
 active_processors_status INTEGER NOT NULL CHECK(active_processors_status BETWEEN 0 AND 3),
 active_processors INTEGER CHECK(active_processors_status<>0 OR active_processors>0),
 PRIMARY KEY(incident_id,sample_index),
 FOREIGN KEY(incident_id,sample_index) REFERENCES system_samples(incident_id,sample_index) ON DELETE CASCADE);
