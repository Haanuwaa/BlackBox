...
[[nodiscard]] telemetry::ProcessFrame process_frame(
    const std::chrono::seconds at,
...
[[nodiscard]] telemetry::ProcessInfo process_info(
    const telemetry::ProcessIdentity identity,
    const char* name) {
...
TEST_CASE("incident snapshot clips history and retains only referenced metadata",
    ...
    core::RecorderSnapshot<telemetry::ProcessFrame> processes{
        3U, {process_frame(0s, unused), process_frame(10s, referenced),
             process_frame(20s, referenced), process_frame(30s, referenced)}};
    ...
    const core::IncidentCaptureWindow window{
        9U, core::MonotonicTimePoint{20s}, core::MonotonicTimePoint{5s},
        core::MonotonicTimePoint{30s}, 1U};

    const auto incident = telemetry::build_incident_snapshot(
        window, core::MonotonicTimePoint{30s}, systems, processes, metadata);
...
TEST_CASE("incident snapshot short uptime starts at the oldest available sample",
    ...
    const core::IncidentCaptureWindow window{
        1U, core::MonotonicTimePoint{55s}, core::MonotonicTimePoint{-65s},
        core::MonotonicTimePoint{60s}, 1U};

    const auto incident = telemetry::build_incident_snapshot(
        window, core::MonotonicTimePoint{60s}, systems, processes, {});
...
TEST_CASE("incident snapshot joins only in-window events into immutable evidence",
    ...
    const core::IncidentCaptureWindow window{
        11U, core::MonotonicTimePoint{20s}, core::MonotonicTimePoint{5s},
        core::MonotonicTimePoint{30s}, 1U};
    ...
    CHECK(incident->system_events()[0].kind ==
          core::SystemEventKind::audio_default_changed);
