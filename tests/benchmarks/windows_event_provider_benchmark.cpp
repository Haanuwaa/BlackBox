...
[[nodiscard]] std::uint64_t parse_or(const char* text, const std::uint64_t fallback) noexcept {
    if (text == nullptr) return fallback;
    const std::string_view input{text};
    std::uint64_t value{};
    const auto parsed = std::from_chars(input.data(), input.data() + input.size(), value);
    return parsed.ec == std::errc{} && parsed.ptr == input.data() + input.size() ? value : fallback;
}
...
int main(int argc, char** argv) {
    ...
    const std::array cases{
        ...
        Case{"update", {false, false, false, false, false, true, false, false, false, false}},
        Case{"application_hang",
    ...
    for (const auto& current : cases) {
        ...
        const auto process_100ns = file_time_value(kernel_after) - file_time_value(kernel_before) +
        ...
        const auto cpu_percent = static_cast<double>(process_100ns) /
                                 (wall_seconds * 10'000'000.0) / static_cast<double>(processors) *
                                 100.0;
        std::fprintf(stderr,
                     ...
                     static_cast<int>(poll_status), static_cast<unsigned long long>(polls),
