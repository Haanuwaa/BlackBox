...
representative_incident(const std::size_t large_text_bytes = 0U) {
    ...
    first.disk_write_bytes_per_second.status =
        core::RecordedValueStatus::temporarily_unavailable;
    first.network_receive_bytes_per_second.status =
        core::RecordedValueStatus::unsupported;
    first.network_transmit_bytes_per_second = {
        2048.25, core::RecordedValueStatus::available};
    first.disk_read_latency_seconds = {0.012, core::RecordedValueStatus::available};
    first.disk_write_latency_seconds.status =
        core::RecordedValueStatus::temporarily_unavailable;
    ...
    first.network_tcp_retransmit_fraction = {0.125,
                                              core::RecordedValueStatus::available};
    first.network_tcp_failed_connections = {3U,
                                             core::RecordedValueStatus::available};
    ...
    first.gpu_dedicated_memory_bytes = {3ULL << 30U,
                                        core::RecordedValueStatus::available};
    first.gpu_shared_memory_bytes = {512ULL << 20U,
                                     core::RecordedValueStatus::available};
    first.foreground_process = {{99U, 123'456U},
                                core::RecordedValueStatus::available};
    ...
    first.cpu_thermal_limit_fraction = {0.8,
                                        core::RecordedValueStatus::available};
    ...
    info.name = {large_text_bytes == 0U ? std::string{"fixture.exe"}
                 ...
                 core::RecordedValueStatus::available};
    info.executable_path = {"C:\\Fixture\\fixture.exe",
                            core::RecordedValueStatus::available};
    ...
    process.disk_write_bytes_per_second = {4096.75,
                                           core::RecordedValueStatus::available};
...
[[nodiscard]] inline std::shared_ptr<const core::IncidentSnapshot> scaled_incident(
    const std::size_t process_count, const std::size_t frame_count,
    const bool inject_system_spike = true) {
    core::IncidentHeader header{};
    header.window.sequence = 7U;
    header.window.event_time = core::MonotonicTimePoint{
        std::chrono::seconds{static_cast<std::int64_t>(frame_count / 2U)}};
    header.window.requested_start = core::MonotonicTimePoint{0s};
    header.window.requested_end = core::MonotonicTimePoint{
        std::chrono::seconds{static_cast<std::int64_t>(frame_count)}};
    ...
    for (std::size_t process_index = 0U; process_index < process_count; ++process_index) {
        core::IncidentProcessInfo info{};
        info.identity = {static_cast<std::uint32_t>(process_index + 1U),
                         process_index + 1'000U};
    ...
    for (std::size_t frame = 0U; frame < frame_count; ++frame) {
        ...
        system.disk_read_bytes_per_second = {1'048'576.0,
                                              core::RecordedValueStatus::available};
