...
TEST_CASE("Windows global hotkey registers and unregisters without elevation",
    ...
    const auto result = manager.register_hotkey(combination, [&activations] { ++activations; });
...
TEST_CASE("default incident hotkey remains Ctrl Shift F12", "[platform][hotkey][contract]") {
    ...
    CHECK_FALSE(platform::default_incident_hotkey.system_modifier);
...
TEST_CASE("Windows global hotkey reports a conflict without disturbing its owner",
    ...
    REQUIRE(owner.register_hotkey(platform::default_incident_hotkey, [&owner_activations] {
        ++owner_activations;
    }) == platform::HotkeyRegistrationResult::registered);
    CHECK(contender.register_hotkey(platform::default_incident_hotkey, [] {}) ==
          platform::HotkeyRegistrationResult::conflict);
    CHECK_FALSE(contender.registered());

    REQUIRE(owner.post_activation_for_testing());
    const auto deadline = std::chrono::steady_clock::now() + 1s;
    while (owner_activations.load() == 0U && std::chrono::steady_clock::now() < deadline) {
        std::this_thread::sleep_for(1ms);
    }
